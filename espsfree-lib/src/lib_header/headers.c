...
FILE * fp;
{
    short len;
/*
 * do not write out generic for esignal_hd_ptr,
 * or pc_wav_hd_ptr
 */
    if (!strcmp(np->name, "esignal_hd_ptr")
...
// NOTE: RLS was a little unclear about how to neatly remove the sphere_to_feasd
// line above, and simply replaced the test with 1.
	 if(1)	{ /* not a NIST either */
           char *def_header;
	   FILE *header_fd;
...
/*
 In this case its not an ESPS, or SIG file, so if DEF_HEADER
...
*/
	   ...
	   if (def_header && (header_fd = fopen(def_header,"rb"))) {
	        if(EspsDebug)
...
/* We do not want to copy certain generic items in copy_header
   ...
   For now, fields "encoding", "esignal_hd_ptr",
...
/*
 * We need to return -1 if the file is a pipe. This is
