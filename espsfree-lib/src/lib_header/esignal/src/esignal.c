...
#define ARCH "Linux"
char	EsignalArch[] = ARCH;   /* Architecture string in native-mode
 ...
 * - FreeFieldSpec
 * - FreeFieldList
 * - FreeAxisNames	(used only in esig_asc.c)
 * - FieldLength
 * - FieldListLength
 * - FindField
 * - GetField		(used only in FindField, higher-level API)
 * - FirstComponent	(used only in FindField, higher-level API)
 * - AddField
 * - AddSubfield
 * - FieldOrder
 * - TypeOrder
 ...
 * Used within the Esignal I/O package, but outside this file (e.g. in
 * esig_asc.c); hence the declaration is non-static.
 ...
 * field list.  Used in FindField.
 * Also used outside this file, in the higher-level API;
 * hence the declaration is non-static.
 * Not part of the public interface.
 */
	 ...
	 char	    *name)	/* component name */
{
    ...
    /* Search NULL-terminated list for field spec with matching name. */
 ...
 * first dot if any or whole string if none.  Used in FindField.
 * Also used outside this file, in the higher-level API;
 * hence the declaration is non-static.
 * Not part of the public interface.
 */
