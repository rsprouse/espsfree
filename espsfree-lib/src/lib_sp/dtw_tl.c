...
#ifndef lint
     static char *sccs_id = "@(#)dtw_tl.c	1.2 10/19/93 ERL";
#endif
...
/*  Find dtw distance between reference and test sequence.
    ...
    test         pointer to array of data test[N]
    ref          pointer to array of data ref[M]
    dim          test and ref are made up of integers which much be in
                 the range [0, dim-1].
    ...
    dist_tbl     used to compute distance between elements in the sequences,
                 i.e. dist_tbl[i][j] is distance between element i and j

    Routine finds mapping w from [0,N-1] to [0,M-1] to minimize
    dist_tbl(test[0], ref[w(0)]) + ... + dist_tbl(test[N-1], ref[w(N-1)]
    (see dtw (3-ESPS)).
...
double *bsofar, **dist_tbl;
{
    ...
    spsassert( test != NULL && ref != NULL && N && M && dist_tbl != NULL,
	...
	dist[0][m].distance = tl_dist( test[0], ref[m], dim , dist_tbl);
    }
    for (n=1; n<N; n++) {
      if (debug_level>3)
	...
	for (m=global[n].lower_lim; m<=global[n].upper_lim; m++) {
		...
		if ( 0.0 == dist[n-1][mprevptr].n_constant ) {
		    if ( dist[n][mptr].distance > dist[n-1][mprevptr].distance) {
			dist[n][mptr].distance =  dist[n-1][mprevptr].distance;
		...
		dist[n][mptr].distance += tl_dist( test[n], ref[m], dim, dist_tbl);
...
double **table;
{
    ...
    spsassert(0<=v1 && v1 < dim && 0<=v2 && v2 < dim,
	      "codeword index and distortion table dimensions incompatible.");
