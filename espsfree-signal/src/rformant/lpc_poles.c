...
/*    lp->type =  SIG_LPC_POLES | P_SHORTS | VAR_REC_SIGNALS; (for binary)*/
 lp->type =  SIG_LPC_POLES | P_DOUBLES | SIG_ASCII | VAR_REC_SIGNALS;
  ...
  if(lp->buff_size >= 1) {
    ...
    for(j=0, init=TRUE, datap=((short**)sp->data)[0]; j < lp->buff_size; j++, datap += step){
	...
	/* add rlpc here */
        ...
        double lpca_temp[MAXORDER];
        int n;
        for (n=0;n<MAXORDER;n++) lpca_temp[n]=lpca[n]; 
	//printf("calling rlpc()\n");
        int rlpcfailed;
	rlpc(lpca,datap,lpc_ord,size,2,&rlpcfailed);
	//printf("called rlpc()\n");
        //if (rlpcfailed) {
	  //printf("rlpc() failed\n");
          //for (n=0;n<MAXORDER;n++) lpca[n]=lpca_temp[n]; 
	  //printf("restored lpca\n");
        //}
        //else {
	  //printf("lpca not restored\n");
        //}
	...
	if(debug & 4) {
		...
		for(k=0; k<pole[j]->npoles; k++)  printf(" %7.1f",pole[j]->freq[k]);
		printf("\n                   ");
		for(k=0; k<pole[j]->npoles; k++)	printf(" %7.1f",pole[j]->band[k]);
		printf("\n");
	}
